...
def manifest_video_id(raw_video_id: str, is_fake: int) -> str | None:
    """Map inference CSV video_id to manifest format (real_000, fake_000_003).

    Inference subdirectories are named:
      - real: bare integer strings ``0``, ``1``, … → ``real_000``, ``real_001``
      - fake: ``000_003``, ``001_870``, … → ``fake_000_003``, ``fake_001_870``

    Also accepts values already in manifest form (``real_000``, ``fake_000_003``).
    Returns None when the id cannot be parsed.
    """
    vid = str(raw_video_id).strip()
    if vid.startswith("real_") or vid.startswith("fake_"):
        return vid
    if is_fake == 0:
        try:
            return f"real_{int(vid):03d}"
        except ValueError:
            return None
    if is_fake == 1:
        return f"fake_{vid}"
...
def _summarize_results_dir(results_dir: Path) -> pd.DataFrame:
    ...
    for csv_path in sorted(results_dir.glob("*.csv")):
        ...
        raw_id = last["video_id"] if "video_id" in last.index else csv_path.stem
        video_id = manifest_video_id(raw_id, is_fake) or raw_id
