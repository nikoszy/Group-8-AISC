...
from src.mrl.inference import load_model, process_video, resolve_device
...
def derive_is_fake(video_path: Path) -> int:
    ...
    Returns:
        1  if any path component contains "manipulated" or "fake"
        0  if any path component contains "original" or "real"
       -1  otherwise (unknown)
    """
    parts = video_path.resolve().as_posix().lower()
    if "manipulated" in parts or "fake" in parts:
        return 1
    if "original" in parts or "real" in parts:
        return 0
    return -1
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="Force device: cpu, cuda, or directml. Auto-detected if omitted.",
    )
    p.add_argument(
        "--skip-existing",
        action="store_true",
        help="Skip videos that already have a CSV in --output-dir.",
    )
    p.add_argument(
        "-v", "--verbose",
        action="store_true",
...
def main() -> None:
    ...
    existing_csv = {
        p.stem for p in output_dir.glob("*.csv")
    }
    ...
    for vdir in video_dirs:
        ...
        if args.skip_existing and vdir.name in existing_csv:
            logging.info(
                "Skipping %s — results already exist at %s",
                vdir.name, csv_path,
            )
            continue
