...
Field notes:
- `combined_score` — quality-weighted mean P(fake) across detected face frames [0, 1]
...
- `module_scores.temporal` — std-dev of per-frame prob_fake (frame inconsistency signal)
- `per_frame[].quality` — Laplacian sharpness score used as quality proxy [0, 1]
