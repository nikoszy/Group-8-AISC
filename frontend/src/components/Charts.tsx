...
export function Charts({ frames }: ChartsProps) {
    ...
    <div>
      ...
      <ResponsiveContainer width="100%" height={280}>
        <ComposedChart data={data} margin={{ top: 4, right: 16, left: -8, bottom: 0 }}>
          ...
          <ReferenceLine y={0.6} stroke="rgba(255,65,54,0.4)"  strokeDasharray="4 4" />
          <ReferenceLine y={0.4} stroke="rgba(46,204,113,0.4)" strokeDasharray="4 4" />
      ...
      <div className="flex gap-6 mt-4 flex-wrap" style={{ fontSize: '10px', fontFamily: '"DM Mono", monospace', color: 'var(--muted)' }}>
        <span style={{ color: 'rgba(255,65,54,0.6)' }}>— — FAKE threshold (0.6)</span>
        <span style={{ color: 'rgba(46,204,113,0.6)' }}>— — REAL threshold (0.4)</span>
      </div>
    </div>
  )
}
