...
function heatmapBg(value: number): string {
  if (value < 0.2) return 'rgba(39, 174, 96, 0.20)'
  if (value < 0.4) return 'rgba(169, 216, 110, 0.20)'
  if (value < 0.6) return 'rgba(241, 196, 15, 0.20)'
  if (value < 0.8) return 'rgba(230, 126, 34, 0.20)'
  return 'rgba(192, 57, 43, 0.25)'
}

function heatmapText(value: number): string {
  if (value < 0.2) return '#2ECC71'
...
export function PerFrameScores({ frames }: PerFrameScoresProps) {
  return (
          ...
          {frames.map((f) => (
            ...
            >
              ...
              <td
                ...
                style={{
                  ...COL_STYLE,
                  backgroundColor: f.face_detected ? heatmapBg(f.prob_fake) : 'transparent',
                  color: f.face_detected ? heatmapText(f.prob_fake) : 'var(--muted)',
                  fontWeight: 500,
                }}
              >
