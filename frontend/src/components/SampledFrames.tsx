...
function probColor(p: number, verdictHi: number, verdictLo: number): string {
  if (p < verdictLo) return 'var(--real)'
  if (p < verdictHi) return 'var(--uncertain)'
  return 'var(--fake)'
}
        ...
        {frame.face_detected && (
          <div
            ...
            style={{
              ...
              color: probColor(frame.prob_fake, verdictHi, verdictLo),
              border: `1px solid ${probColor(frame.prob_fake, verdictHi, verdictLo)}40`,
            }}
          >
...
export function SampledFrames({ frames, verdictHi = 0.6, verdictLo = 0.4 }: SampledFramesProps) {
