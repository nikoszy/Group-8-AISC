...
export function Verdict({
  ...
  const isExtremeThreshold = threshold <= 0.15 || threshold >= 0.85
  const collapsedPrediction = typeof temporalScore === 'number' && temporalScore < 0.01
  ...
  return (
    ...
    >
      ...
      {/* Confidence band */}
      <ConfidenceBand probFake={probFakeMean} verdictHi={verdictHi} verdictLo={verdictLo} />
      ...
      <div className="flex flex-wrap gap-6 mt-8 pt-6" style={{ borderTop: '1px solid var(--border-dim)' }}>
        ...
        <div>
          ...
          <p className="font-jbmono font-medium text-sm mt-0.5" style={{ color: 'var(--text)' }}>
            {modelUsed === 'ensemble_learned'
              ? 'LogReg (trained)'
              : modelUsed === 'cnn_fallback'
                ? 'CNN fallback'
                : modelUsed === 'cnn_fallback_degraded'
                  ? 'CNN fallback (degraded)'
                  : 'Equal weights'}
          </p>
        </div>
      </div>

      {(isExtremeThreshold || collapsedPrediction) && (
        <div
        ...
        >
          {isExtremeThreshold && (
            <p className="font-dmmono text-xs" style={{ color: 'var(--muted)' }}>
              Warning: threshold is extreme ({threshold.toFixed(3)}); calibration drift can skew class balance.
            </p>
          )}
          {collapsedPrediction && (
            <p className="font-dmmono text-xs mt-1" style={{ color: 'var(--muted)' }}>
              Warning: frame-level scores are nearly constant (low temporal spread).
            </p>
          )}
        </div>
      )}
