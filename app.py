...
def _load_cnn_alpha(bundle_path=STACK_BUNDLE_PATH, fallback=0.65):
    """
    Return the learned CNN blend weight alpha from the stacking bundle.

    alpha is used as:
        frame_prob = alpha * CNN_prob + (1 - alpha) * LR_prob

    Falls back to 0.65 if the bundle is missing, corrupt, or flagged unreliable.
    """
...
def detect_face(frame, min_face_frac=0.03):
...
def process_video(video_path, n_frames, lr_bundle, cnn_model, min_quality):
    ...
    for i, idx in enumerate(indices):
        ...
        crop, bbox = detect_face(frame)

        # Build thumbnail with bounding box
        ...
        artifact, fft, lap, ear = extract_features(crop)
        x_sc   = lr_scaler.transform(np.array([[artifact, fft, lap, ear]],
                                               dtype=np.float32))
    ...
    # Temporal consistency — dense consecutive frame burst for optical flow
    ...
    if temporal_result and temporal_result.get("available"):
        combined = combined + 0.15 * (temporal_result["score"] - combined)
    if rppg_result and rppg_result.get("available"):
        combined = combined + 0.10 * (rppg_result["fake_score"] - combined)
