...
#
# LABEL CONVENTION
#   0 = REAL  (data/archive/FaceForensics/original/)
#   1 = FAKE  (data/archive/FaceForensics/Deepfakes/)
# =============================================================================
...
FF_DIR = os.environ.get("FF_DIR", os.path.join("data", "FaceForensics++_C23"))
REAL_SUBDIR = os.environ.get("REAL_SUBDIR", "original")
FAKE_SUBDIR = os.environ.get("FAKE_SUBDIR", "Deepfakes")

REAL_SRC  = os.path.join(FF_DIR, REAL_SUBDIR)
FAKE_SRC  = os.path.join(FF_DIR, FAKE_SUBDIR)

REAL_DIR      = os.environ.get("REAL_DIR", os.path.join("data", "real", "frames"))
FAKE_DIR      = os.environ.get("FAKE_DIR", os.path.join("data", "fake", "frames"))
MANIFEST_PATH = os.environ.get("MANIFEST_PATH", os.path.join("data", "manifest.csv"))
VIDEO_EAR_CSV = os.environ.get("VIDEO_EAR_CSV", os.path.join("data", "video_ear_scores.csv"))
WRITE_VIDEO_EAR_SCORES = os.environ.get("WRITE_VIDEO_EAR_SCORES", "1").strip().lower() not in {"0", "false", "no"}
...
def build_manifest_video_id(prefix, vid_stem):
    """
    Build stable manifest IDs for downstream source-video resolution.

    REAL examples:
        000 -> real_000
    FAKE examples (FF++ Deepfakes):
        000_003 -> fake_000_003
    """
    cleaned = (vid_stem or "").strip()
    if prefix == "fake":
        m = re.fullmatch(r"(\d{3})_(\d{3})", cleaned)
        if m:
            return f"fake_{m.group(1)}_{m.group(2)}"
...
def extract_class(src_dir, videos, out_dir, prefix, label, target_n, fps):
    ...
    for vid_name in videos[:needed]:
        ...
        vid_id   = os.path.splitext(vid_name)[0]          # e.g. "000" or "000_003"
        ...
        for _, crop in faces:
            ...
            if cv2.imwrite(path, crop):
                rows.append({
                    "file_path"     : path,
                    "label"         : label,
                    "video_id"      : build_manifest_video_id(prefix, vid_id),
                    "source_dataset": f"archive/FaceForensics/{src_dir.split(os.sep)[-1]}",
                })
                saved += 1
...
# ---------------------------------------------------------------------------
# Step 5b: Optional video-level EAR scores (Module 1)
# ---------------------------------------------------------------------------
if WRITE_VIDEO_EAR_SCORES and all_rows:
    print("-" * 65)
    print(f"MODULE 1 EAR  ->  {VIDEO_EAR_CSV}")
    print("-" * 65)
    from src.blink_analysis.ear_scorer import write_video_ear_scores_csv

    jobs = [(r["video_id"], r["source_dataset"]) for r in all_rows]
    write_video_ear_scores_csv(jobs, path=VIDEO_EAR_CSV, verbose=True)
    print()
