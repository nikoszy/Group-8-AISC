...
def _load_cnn_alpha(bundle_path=STACK_BUNDLE_PATH, fallback=0.65):
    """
    Return the CNN blend weight alpha from the stacking bundle.

    alpha is the fraction of the CNN score used in:
        frame_prob = alpha * CNN_prob + (1 - alpha) * LR_prob

    A value of 0.20 means 20% CNN + 80% LR.  This is counter-intuitive
    (the CNN has much higher AUC), but it is the blend that maximises
    the *combined* AUC — the LR adds calibration, not raw accuracy.

    Args:
        bundle_path : path to stacking_bundle.pkl
        fallback    : alpha to use if the bundle is missing or unreliable
...
def detect_face_crop(frame, min_face_frac=0.03):
    """
    Returns (crop_224, bbox) or (None, None).
...
def extract_features(crop_bgr):
    ...
    ear_score is fixed at 0.5 at inference time (Module 1 MRL not run live).
    The saved scaler and model expect exactly 4 features.
    """
...
def predict_video(video_path, lr_bundle, cnn_model=None,
    ...
    frame_results = []
    face_crops    = []  # for temporal / rPPG

    for i, frame in enumerate(frames):
        crop, bbox = detect_face_crop(frame)
        if crop is None:
    ...
    # ---- Temporal consistency (dense consecutive frames) ----
    # Optical flow needs consecutive frames (≤ 200ms apart), not the sparse
    # evenly-spaced frames used for CNN/LR.  Re-sample a 2-second burst of
    # consecutive frames from the middle of the video.
    ...
    try:
        ...
        if cap_t.isOpened():
            ...
            fps_t   = cap_t.get(cv2.CAP_PROP_FPS) or fps
            n_burst = min(int(fps_t * 2.0), 60)   # up to 2 s / 60 frames
            start   = max(0, total_t // 2 - n_burst // 2)
            cap_t.set(cv2.CAP_PROP_POS_FRAMES, start)
            burst_frames = []
            ...
            cap_t.release()
            temporal_result = temporal_consistency_score(burst_frames)
    except Exception:
        pass
    ...
    try:
        ...
        rppg_result = rppg_score(valid_crops, fps=fps)
    except Exception:
        pass
    ...
    if temporal_result and temporal_result.get("available"):
        temporal_fake = temporal_result["score"]
        combined = combined + 0.15 * (temporal_fake - combined)
        signal_count += 1

    if rppg_result and rppg_result.get("available"):
        rppg_fake = rppg_result["fake_score"]
        combined = combined + 0.10 * (rppg_fake - combined)
        signal_count += 1
